...
import datetime
import peewee as pw
from states.searching_states import HighPriceStates
...
@bot.callback_query_handler(func=lambda call: call.data == 'high_price')
def handle_highprice_query(call: CallbackQuery):
    user_id = call.from_user.id
    user_name = call.from_user.username
    print(user_id)
    print(user_name)
    handle_highprice_city(call.message, user_id, user_name)


def handle_highprice_city(message: Message, user_id, user_name) -> None:
    """Получение города от пользователя"""
    bot.set_state(user_id, HighPriceStates.city, message.chat.id)
    bot.send_message(user_id, f'Привет, {user_name} в '
                              f'каком городе будем искать самые дорогие отели?')


@bot.message_handler(state=HighPriceStates.city)
def get_count(message: Message) -> None:
    if message.text.isalpha():
        bot.send_message(message.chat.id,
                         'Теперь введите количество отелей которые хотите увидеть.')
        bot.set_state(message.from_user.id, HighPriceStates.hotels, message.chat.id)
...
@bot.message_handler(state=HighPriceStates.hotels)
def get_check_in_date(message: Message) -> None:
    bot.send_message(message.from_user.id,
                     'Теперь введите желаемую дату заезда.(в формате число-месяц-год, 15-09-23)')
    bot.set_state(message.from_user.id, HighPriceStates.check_in_date, message.chat.id)
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
...
@bot.message_handler(state=HighPriceStates.check_in_date)
def get_check_out_date(message: Message) -> None:
    bot.send_message(message.from_user.id,
                     'Теперь введите желаемую дату выезда.(в формате число-месяц-год, 20-09-23)')
    bot.set_state(message.from_user.id, HighPriceStates.check_out_date, message.chat.id)
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
...
@bot.message_handler(state=HighPriceStates.check_out_date)
...
def property_search(message: Message, gaia_id=str, country=str,
                    city=str, hotel_count=int, day_in=int, month_in=int,
                    year_in=int, day_out=int, month_out=int, year_out=int):
    ...
    print('Строка 215: ', response.status_code)
